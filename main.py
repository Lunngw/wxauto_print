...
root.iconphoto(True, tkinter.PhotoImage(file='icon64_wx_logo.png'))
# 预设发送人
name1 = []
name_list1 = open('list/list1.txt', 'r', encoding='utf-8')
for line in name_list1:
    name1.append(line.strip())
name2 = []
name_list2 = open('list/list2.txt', 'r', encoding='utf-8')
for line in name_list2:
    name2.append(line.strip())
name3 = []
name_list3 = open('list/list3.txt', 'r', encoding='utf-8')
for line in name_list3:
    name3.append(line.strip())
# 选择联系人ui
lbl1 = Label(root, text='------请选择联系人/群------')
lbl1.grid(column=0, row=0, sticky=W, columnspan=2)
text_box_name = tkinter.Spinbox(root, wrap=True, values=('预设一', '预设二', '预设三'))
text_box_name.grid(column=0, row=1, sticky=W, columnspan=2)


# 图片ui
def img():
    global path, img_open, img_
    num = 0
    path = tkinter.filedialog.askopenfilenames()
    for img1 in path:
        num += 1
        img_open = Image.open(img1)
        img_open = img_open.resize((300, 300))
        img_ = ImageTk.PhotoImage(img_open)
        label_img = tkinter.Label(root, image=img_)
        label_img.grid(column=2, row=0, rowspan=7)


lal2 = Label(root, text='------请选择图片------')
lal2.grid(column=0, row=2, sticky=W, columnspan=2)
button_img = tkinter.Button(root, text='选择图片', command=img)
button_img.grid(column=0, row=3, sticky=W, columnspan=2)

# 输入文本ui
lbl3 = Label(root, text='----请输入所要发送的文本----')
lbl3.grid(column=0, row=5, sticky=W, columnspan=2)
text_box_out = scrolledtext.ScrolledText(root, width=30, height=10, )
text_box_out.grid(column=0, row=6, columnspan=2)


# 清除文本ui
def delete():
    text_box_out.configure(text_box_out.delete(1.0, 'end'))


button_delete = tkinter.Button(root, text='清除', command=delete)
button_delete.grid(column=0, row=7, sticky=W)
...
# 发送
def get():
    text_out = text_box_out.get(1.0, 'end')
...
# 发送ui
button_post = tkinter.Button(root, text='发送', command=get)
button_post.grid(column=1, row=7, sticky=E)
root.mainloop()
